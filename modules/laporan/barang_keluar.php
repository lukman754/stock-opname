...
$query = "SELECT bk.id, bk.nomor_transaksi, bk.tanggal, bk.keterangan, 
...
if (!empty($lokasi_id)) {
    $query .= " AND bk.lokasi_id = :lokasi_id";
    $params[':lokasi_id'] = $lokasi_id;
}

$query .= " ORDER BY bk.tanggal, bk.id";
...
<body>
    ...
    <p class="text-center">Tidak ada data barang keluar pada periode tersebut.</p>
    <?php else: ?>
    <table>
        <thead>
            <tr>
        ...
        <tbody>
            <?php $no = 1; foreach ($barang_keluar_list as $bk): ?>
            <tr>
                ...
                <td>
                    ...
                    $query_detail = "SELECT bkd.jumlah_utuh, bkd.jumlah_pecahan, 
                                    b.nama_barang, su.nama_satuan as satuan_utuh, 
                    ...
                    $stmt_detail->bindParam(':id', $bk['id']);
                    $stmt_detail->execute();
                    ...
                    foreach ($details as $index => $detail) {
                        ...
                        $detail_text .= $detail['nama_barang'] . ' (' . $detail['jumlah_utuh'] . ' ' . $detail['satuan_utuh'];
                        if ($detail['jumlah_pecahan'] > 0 && !empty($detail['satuan_pecahan'])) {
                            $detail_text .= ', ' . $detail['jumlah_pecahan'] . ' ' . $detail['satuan_pecahan'];
                        }
                        $detail_text .= ')';
                    }
                    echo $detail_text;
